...
if ("fetch" in window) {
  ...
  alert(
    "IE is not supported and I have forgotten about making API calls with XMLHttpRequest"
  );
}

function buildHTMLString(players) {
  const HTMLString = players
    .sort((player1, player2) => player1.Value - player2.Value)
    .map(
      (player) => `
    <div class="card">
                <img src = "./images/player-images/${player.Id}.jpg" alt = "${
        player.Id
      }"/>
                ...
                } vs ${player.UpComingMatchesList[0].VsCCode || ""}</p>
                <p>Match Date : ${
...
function searchPlayers(inputString) {
  ...
  if (inputString.trim() === "") matchedPlayers = [...players];
    ...
    matchedPlayers = players.filter(
      (player) =>
        player.TName.toLowerCase().includes(searchString) ||
...
document
  ...
  .addEventListener("input", debounce(searchPlayers, 800));
